...
class ImageToWordApp:
    ...
    def add_image_controls(self, frame, title):
        ...
        OptionMenu(title_frame, layout_var, "Single Column", "Two Columns").pack(side="left", padx=5)
    ...
    1. **Select a Folder**:
    - Click on 'Select Folder' to choose a directory containing your images (formats: PNG, JPG, JPEG, GIF, BMP).

    2. **Preview Images**:
    - A preview window will open displaying the selected images.
    - You can click on an image to view it in full size.

    3. **Set Maximum Image Width**:
    - Enter the desired maximum image width in inches in the provided entry box.

    4. **Add Notes**:
    - For each image group, you can add notes in the text box provided below each image.

    5. **Choose Layout**:
    - Select the desired layout option for images (Single Column or Two Columns) from the dropdown menu next to each title.

    6. **Save/Apply Settings**:
    - Click 'Save/Apply' to apply the settings and return to the main menu.

    7. **Generate Document**:
    - Once you're done, click on 'Generate Document' to create a Word file containing your images and notes.
    - If a document with the same name already exists, you will be prompted to overwrite it.

    8. **Feedback**:
    - Click on 'Feedback' to provide suggestions or report issues.

    For further assistance, please contact support.
    """
        Label(help_window, text=help_text, justify='left', font=("Arial", 12)).pack(padx=10, pady=10)
